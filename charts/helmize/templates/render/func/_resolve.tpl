  ...
  {{- if (not (include "lib.utils.errors.unmarshalingError" $conds)) -}}
    ...
    {{- if $paths -}}
      ...
      {{- if (not (include "lib.utils.errors.unmarshalingError" $files)) -}}
        ...
        {{- end -}}  
      {{- else -}}
        {{- include "lib.utils.errors.fail" (printf "Template inventory.render.func.files.finder Returned invalid YAML:\n%s" (toYaml $func_files | nindent 2)) -}}
      {{- end -}}
    {{- end -}}
  {{- else -}}
    {{- include "lib.utils.errors.fail" (printf "Template inventory.conditions.func.resolve returned invalid YAML:\n%s" $conds) -}}
  {{- end -}}
