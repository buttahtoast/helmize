...
{{- define "helmize.render.func.resolve" -}}
  ...
  {{- if (not (include "lib.utils.errors.unmarshalingError" $conds)) -}}
    ...
    {{- if $paths -}}
      ...
      {{- $func_files := (dict "tpl" "helmize.render.func.finder" "ctx" (dict "paths" $paths "ctx" $ctx)) -}}
      ...
      {{- if (not (include "lib.utils.errors.unmarshalingError" $files)) -}}
        ...
        {{- if $files.files -}}
          ...
          {{/* Execute File Train */}}
          {{- $train := fromYaml (include "helmize.render.func.train" (dict "files" $files.files "groups" $.groups "ctx" $ctx "ts" $return)) -}}
        ...
        {{- end -}}  
      {{- else -}}
        {{- include "lib.utils.errors.fail" (printf "Template helmize.render.func.finder Returned invalid YAML:\n%s" (toYaml $func_files | nindent 2)) -}}
      {{- end -}}
    {{- end -}}
  {{- else -}}
