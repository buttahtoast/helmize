...
{{- define "helmize.entrypoint.func.deploy" -}}
  ...
  {{- if ((fromYaml (include "lib.utils.dicts.get" (dict "data" $.Values "path" (include "helmize.entrypoint.defaults.show_config" $)))).res) -}}
  
     {{- toYaml (dict "config" (fromYaml (include "helmize.config.func.get" $))) | nindent 0  }}

  {{/* Print Summary Output */}}
  {{- else if ((fromYaml (include "lib.utils.dicts.get" (dict "data" $.Values "path" (include "helmize.entrypoint.defaults.summary_value" $)))).res) -}}
  ...
  {{- else -}}
    {{/* Resolve Files */}}
    {{- $train_raw := include "helmize.entrypoint.func.resolve" $ -}}
