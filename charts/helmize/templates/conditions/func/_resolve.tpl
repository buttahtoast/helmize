  ...
  {{- if $.ctx -}}
    ...
    {{- if $conditions -}}
      ...
      {{- range $condition := $conditions.conditions -}}
        ...
        {{- $key := "" -}}
        {{- if $value_key -}}
          {{- $key = (fromYaml (include "lib.utils.dicts.lookup" (dict "data" $.ctx.Values "path" $value_key "required" (default false (get $condition (include "inventory.conditions.defaults.conditions.required" $)))))).res -}}
        {{- end -}}
        
        {{/* Assign Default if available */}}
        {{- if and (not $key) (get $condition (include "inventory.conditions.defaults.conditions.default" $)) -}}
          {{- $key = (get $condition (include "inventory.conditions.defaults.conditions.default" $)) -}}
        ...
        {{/* Run Templates */}}
        {{- if (get $condition (include "inventory.conditions.defaults.conditions.tpls" $)) -}}
          {{- include "inventory.conditions.func.resolve.templates" (dict "tpls" (get $condition (include "inventory.conditions.defaults.conditions.tpls" $)) "condition" $condition_struct "ctx" $.ctx) -}}
        {{- end -}}
        {{- $shared_data = (mergeOverwrite $shared_data (default dict (get $condition (include "inventory.conditions.defaults.conditions.data" $)))) -}}
...
{{- define "inventory.conditions.func.resolve.templates" -}}
  {{- range $t := $.tpls -}}
