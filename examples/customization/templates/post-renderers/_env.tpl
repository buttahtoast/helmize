{{- define "customization.postrenderers.env" -}}
    ...
    {{- if (has $kind (list "deployment" "statefulset" "daemonset")) -}}
      ...
      {{- if (eq (default "false" (fromYaml (include "lib.utils.dicts.lookup" (dict "path" $label "data" $.content))).res) "true") -}}
        ...
        {{- range $m := $.content.spec.template.spec.containers -}}
          {{- include "lib.utils.dicts.merge" (dict "base" $m "data" (fromYaml (include "customization.postrenderers.env.values" $)) "ctx" $.ctx) -}}
...
{{- define "customization.postrenderers.env.values" -}}
env:
- __inject__
- name: EXISTING_VAR
